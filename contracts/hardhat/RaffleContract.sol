 ...
 * @dev Raffle contract with per-raffle ShelliesPoints pricing.
 *      Each raffle stores its own pointsPerTicket at creation time,
 *      mirroring the off-chain points_per_ticket column in the DB.
 *      All other limits (max_tickets_per_user, max_participants, end_date)
 *      are enforced by the backend API — the contract handles prize custody,
 *      points spending, and winner selection only.
    ...
    Pausable
{
    ...
    struct Raffle {
        address prizeToken;        // NFT/Token contract address     (20 bytes)
        uint96  prizeTokenId;      // NFT ID or ERC20 amount         (12 bytes) — slot 1
        uint96  pointsPerTicket;   // ShelliesPoints cost per ticket (12 bytes)
        RaffleState state;         // Current state                  ( 1 byte)
        bool    isNFT;             // NFT vs ERC20 prize             ( 1 byte)  — slot 2
        address winner;            // Winner address                 (20 bytes) — slot 3
    }

    mapping(uint256 => Raffle) public raffles;

    /// @notice The deployed ShelliesPoints contract used for ticket payment.
    ...
    /**
     * @notice Deposit an NFT prize and create the raffle in CREATED state.
     * @param raffleId        Must match the sequential ID in the database.
     * @param prizeToken      ERC-721 contract address.
     * @param tokenId         NFT token ID to escrow.
    ...
    /**
     * @notice Deposit an ERC-20 prize and create the raffle in CREATED state.
     * @param _pointsPerTicket ShelliesPoints cost per ticket (mirrors DB points_per_ticket).
     */
    function createRaffleWithToken(
        ...
        emit RaffleCreated(raffleId, prizeToken, amount, false, _pointsPerTicket);
        emit RaffleStateChanged(raffleId, RaffleState.CREATED, RaffleState.CREATED);
    }

    /// @notice Move raffle from CREATED → ACTIVE.
    function activateRaffle(uint256 raffleId)
        external onlyAdmin raffleExists(raffleId) raffleInState(raffleId, RaffleState.CREATED)
    {
    ...
    /**
     * @notice Deposit an NFT prize and immediately activate the raffle.
     * @param _pointsPerTicket ShelliesPoints cost per ticket (mirrors DB points_per_ticket).
     */
    ...
    /**
     * @notice Deposit an ERC-20 prize and immediately activate the raffle.
     * @param _pointsPerTicket ShelliesPoints cost per ticket (mirrors DB points_per_ticket).
     */
     ...
     * @param raffleId    The raffle to enter (must match DB id).
     * @param ticketCount Number of tickets to purchase (> 0).
     */
    function joinRaffle(
        uint256 raffleId,
        uint256 ticketCount
        ...
        // Spend points — reverts if user has insufficient balance
    ...
    /// @notice Returns the total ShelliesPoints cost for a given number of tickets in a raffle.
    ...
    function _transferPrize(uint256 raffleId, address winner) internal {
        ...
        if (r.isNFT) {
            ...
            IERC20(r.prizeToken).transfer(winner, r.prizeTokenId);
        }
    }
}
